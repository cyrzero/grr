...
function begin_page($title, $page = "with_session")
{
	...
	if (@file_exists('admin_accueil.php')){ // Si on est dans l'administration
		...
		{
			$a .= '<link rel="stylesheet" type="text/css" href="../'.$sheetcss.'/select2.css" />'.PHP_EOL;
			$a .= '<link rel="stylesheet" type="text/css" href="../'.$sheetcss.'/select2-bootstrap.css" />'.PHP_EOL;
			$a .= '<link rel="stylesheet" type="text/css" href="../'.$sheetcss.'/bootstrap-multiselect.css">'.PHP_EOL;
			$a .= '<link rel="stylesheet" type="text/css" href="../'.$sheetcss.'/bootstrap-clockpicker.min.css">'.PHP_EOL;
		}
		if ((isset($_GET['pview'])) && ($_GET['pview'] == 1))
			$a .= '<link rel="stylesheet" type="text/css" href="../themes/print/css/style.css" />'.PHP_EOL;
			$a .= '<link rel="stylesheet" type="text/css" href="../'.$sheetcss.'/jquery-ui.css" />'.PHP_EOL;
			$a .= '<link rel="stylesheet" type="text/css" href="../'.$sheetcss.'/jquery-ui-timepicker-addon.css" >'.PHP_EOL;
	...
	$a .= '<div id ="container" class="container-fluid">'.PHP_EOL;
	$a .= '<div class="row">'.PHP_EOL;
	return $a;
}
...
function print_header($day = '', $month = '', $year = '', $type_session = 'with_session')
{
	...
	if ((!isset($_GET['pview'])) || ($_GET['pview'] != 1))
	{
		...
		if (!(isset($desactive_bandeau_sup) && ($desactive_bandeau_sup == 1) && ($type_session != 'with_session')))
		{
			...
			$year  = date("Y",$date_);
			//aa
			echo ' <div class="col-lg-12">',PHP_EOL,'<div id="panel">',PHP_EOL,'<div class="content">',PHP_EOL,'<table id="header">',PHP_EOL,'<tr>',PHP_EOL;
			//Logo
			...
			echo '<a id="open" class="open" href="#"></a>',PHP_EOL,'</ul>',PHP_EOL,'</div>',PHP_EOL,'</div>',PHP_EOL;
			echo '</div>',PHP_EOL;
			//aa
		}
	}
...
function make_room_item_html($link, $current_area, $current_room, $year, $month, $day)
{
	...
	if ($res)
	{
		for ($i = 0; ($row = grr_sql_row($res, $i)); $i++)
		{
			...
			{
				$link2 = $link.'.php?year='.$year.'&amp;month='.$month.'&amp;day='.$day.'&amp;room='.$row[0];
				//$link2 = $link.'.php?year='.$year.'&amp;month='.$month.'&amp;day='.$day.'&amp;room='.$row[1];
...
function send_mail($id_entry, $action, $dformat, $tab_id_moderes = array())
{
	...
	$mail->Port = Settings::get("grr_mail_port");
	$mail->SMTPSecure = Settings::get("grr_mail_encrypt"); 
	$mail->SMTPAuth = false;
	$mail->CharSet = 'UTF-8';
...
// créer les selecteurs de date
 function jQuery_DatePicker($typeDate)
 {
 	// si appelé avec argument 'rep_end'
 	...
 	if ($typeDate == 'rep_end' && isset($_GET['id']))
 	{
 		...
 		if (grr_sql_count($res) == 1)
 		{
 			//extrait la date de fin de périodicité
 		...
 		{
 			// si aucun résultat, récupère la date passée en valeur ou la date du jour.
 			if (isset ($_GET['day']))
 				$day = $_GET['day'];
 			else
 				$day = date("d");
...
// vérifie qu'une chaine de caractère est composée uniquement de chiffres
function checkDigit($itemToCheck){
	if (ctype_digit($itemToCheck)){
		header("Location: traitementcontact.php");
	}else{
		echo('<script>document.getElementById("telephone").value="numéro invalide"</script>');
	}
}
