...
function overloadWithRoom($id_area, $id_room)
{

	// si l'id de l'area n'est pas précisé, on cherche tous les champs additionnels
	if ($id_area == ""){
		echo"dans if";
		$sqlstring = "SELECT fieldname ,fieldtype, ".TABLE_PREFIX."_overload.id, fieldlist, ".TABLE_PREFIX."_area.area_name, 
							affichage, overload_mail, ".TABLE_PREFIX."_overload.obligatoire, ".TABLE_PREFIX."_overload.confidentiel 
					FROM ".TABLE_PREFIX."_overload, ".TABLE_PREFIX."_area 
					WHERE(".TABLE_PREFIX."_overload.id_area = ".TABLE_PREFIX."_area.id) 
					ORDER BY fieldname,fieldtype ";
	}else{
		//echo "dans else";

		$sqlstring="SELECT fieldname,fieldtype, grr_overload.id, fieldlist, affichage, overload_mail, obligatoire, 
						confidentiel 
					FROM ".TABLE_PREFIX."_overload,".TABLE_PREFIX."_room 
					WHERE ".TABLE_PREFIX."_overload.id_room = ".TABLE_PREFIX."_room.id 
						AND id_room='".$id_room."'
					union
					SELECT DISTINCT fieldname,fieldtype, grr_overload.id, fieldlist, affichage, overload_mail, obligatoire, 
							confidentiel 
					FROM ".TABLE_PREFIX."_overload,".TABLE_PREFIX."_room  
					WHERE ".TABLE_PREFIX."_room.area_id=".TABLE_PREFIX."_overload.id_area 
						AND ".TABLE_PREFIX."_overload.id_area='".$id_area."'
						and ".TABLE_PREFIX."_overload.id_room IS NULL
					ORDER BY fieldname,fieldtype";
					//echo $sqlstring;
