...
$sql = "SELECT id, type_name, order_display, couleur, type_letter FROM ".TABLE_PREFIX."type_area
...
if (isset($_GET['valider']))
{
	...
	if ($res)
	{
		for ($i = 0; ($row = grr_sql_row($res, $i)); $i++)
		{
			...
			{
				$nb_types_valides ++;
				$del = grr_sql_query("DELETE FROM ".TABLE_PREFIX."j_type_area WHERE id_area='".$id_area."' AND id_type = '".$row[0]."'");
			}
			else
			{
				$type_si_aucun = $row[0];
