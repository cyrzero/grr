...
else
{
	...
	for ($i = 0; ($row = grr_sql_row($res, $i)); $i++)
	{
		...
		if (verif_acces_ressource(getUserName(), $id_room[$i]))
		{
			...
			$a = $a + 1;
			echo '><a id="afficherBoutonSelection'.$a.'" class="lienPlanning" href="#afficherBoutonSelection" onclick="afficherMoisSemaine('.$a.')" style="display:inline;">'.htmlspecialchars($row['0']).'</a>'.PHP_EOL;
			echo '<a id="cacherBoutonSelection'.$a.'" class="lienPlanning" href="#afficherBoutonSelection" onclick="cacherMoisSemaine('.$a.')" style="display:none;">'.htmlspecialchars($row['0']).'</a>'.PHP_EOL;
